...
namespace Templite\Cms\Http\Requests\TemplateField;
...
class StoreTemplateFieldRequest extends FormRequest
{
        ...
        $rules = [
            ...
            'key' => [
                ...
                Rule::unique('block_fields')
                    ->where('fieldable_type', 'template_page')
                    ->where('fieldable_id', $templateId)
    ...
    public function messages(): array
    {
        return [
            ...
            'key.unique' => 'Такой ключ уже существует в этом шаблоне (на данном уровне вложенности).',
            'key.not_in' => 'Этот ключ зарезервирован системой.',
            'parent_id.exists' => 'Родительское поле должно быть типа array и принадлежать тому же шаблону.',
        ];
    }
}
